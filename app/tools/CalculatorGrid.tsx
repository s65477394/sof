...
// 这是全新的、功能完整的税务计算器组件
// 我们把所有的逻辑（useState, calculateTax）和UI（return）都放进了这一个函数里
function TaxCalculator() {
  const [income, setIncome] = useState('');
  const [deductions, setDeductions] = useState('');
  // 我们明确告诉程序，result盒子里会有这三个东西
  const [result, setResult] = useState<{ tax: number; taxableIncome: number; afterTaxIncome: number } | null>(null);
  ...
  const calculateTax = () => {
    ...
    const taxableIncome = incomeNum - deductionsNum;
    let tax = 0;
    ...
    setResult({
      ...
      tax: Math.max(0, tax),
      afterTaxIncome: Math.max(0, afterTaxIncome)
    });
  };
...
function InvestmentCalculator() {
  ...
  const [result, setResult] = useState<{ futureValue: number; totalReturn: number; annualReturn: number; } | null>(null);
  ...
  const calculateInvestment = () => {
    ...
    setResult({
      futureValue,
      totalReturn,
      annualReturn: t > 0 ? totalReturn / t : 0
    });
  };
      ...
      {result && (
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <div className="text-2xl font-bold text-green-600">
              ¥{result.totalReturn.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <div className="text-sm text-gray-600">年均收益</div>
            <div className="text-2xl font-bold text-purple-600">
              ¥{result.annualReturn.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
          </div>
        </div>
      )}
...
// 这是最外面的“大房子”，它保持不变，但现在它可以正确地找到里面的小房间了
export default function CalculatorGrid() {
  const [activeCalculator, setActiveCalculator] = useState('tax');
  
  const calculators = [
    { id: 'tax', title: '税务筹划计算器', description: '计算税务优化方案的节税效果', icon: 'ri-calculator-line', color: 'bg-blue-500' },
    { id: 'investment', title: '投资收益计算器', description: '分析投资组合的预期收益', icon: 'ri-line-chart-line', color: 'bg-green-500' },
    { id: 'inheritance', title: '传承成本计算器', description: '评估财富传承的各项成本', icon: 'ri-exchange-line', color: 'bg-purple-500' },
    { id: 'insurance', title: '保险规划计算器', description: '计算保险保障的合理配置', icon: 'ri-shield-check-line', color: 'bg-red-500' }
  ];
